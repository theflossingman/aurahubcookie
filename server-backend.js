...
app.post('/api/cookies/set', (req, res) => {
    ...
    if (!name || value === undefined) {
        return res.status(400).json({ error: 'Cookie name and value required' });
    }
    ...
    try {
        ...
        const cookieOptions = {
            ...
            secure: options.secure || false, // default false for HTTP
            sameSite: options.sameSite || 'Lax'
        };
        
        res.cookie(name, value, cookieOptions);
        
        // Also store in database/file for persistence
        const cookieData = {
            name,
            value,
            timestamp: new Date().toISOString(),
        ...
        console.log(`Cookie set: ${name} and saved to database`);
        res.json({ success: true, message: `Cookie ${name} set and saved to database` });
    } catch (error) {
        console.error('Error setting cookie:', error);
        res.status(500).json({ error: 'Failed to set cookie', details: error.message });
    }
});
...
app.get('/api/cookies/get', (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to get cookies', details: error.message });
    }
});
...
app.post('/api/cookies/delete', (req, res) => {
    ...
    if (!name) {
        return res.status(400).json({ error: 'Cookie name required' });
    }
    ...
    try {
        ...
        console.log(`Cookie deleted: ${name} and removed from database`);
        res.json({ success: true, message: `Cookie ${name} deleted and removed from database` });
    } catch (error) {
        console.error('Error deleting cookie:', error);
        res.status(500).json({ error: 'Failed to delete cookie', details: error.message });
    }
});

// Cookie Clicker API endpoints
app.get('/api/cookie-clicker/:user', (req, res) => {
    try {
        const user = req.params.user;
        const dataPath = path.join(DATA_DIR, 'cookie-clicker-data.json');

        if (fs.existsSync(dataPath)) {
            const data = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
            res.json({ count: data[user] || 0 });
        } else {
            res.json({ count: 0 });
        }
    } catch (error) {
        console.error('Error reading cookie data:', error);
        res.json({ count: 0 });
    }
});

// Get all users' cookie data
app.get('/api/cookie-clicker-all', (req, res) => {
    try {
        const dataPath = path.join(DATA_DIR, 'cookie-clicker-data.json');
        ...
        console.error('Error reading all cookie data:', error);
        res.status(500).json({ error: 'Failed to read cookie data' });
    }
});

// Attack user - atomically transfer cookies
app.post('/api/cookie-clicker-attack', (req, res) => {
    try {
        const { attacker, target, amount } = req.body;
        const dataPath = path.join(DATA_DIR, 'cookie-clicker-data.json');

        console.log(`💣 ATTACK REQUEST: ${attacker} attacking ${target} for ${amount} cookies`);
        ...
        if (fs.existsSync(dataPath)) {
            ...
            console.log(`   Data file does not exist, creating new`);
        }

        const targetCount = data[target] || 0;
        const attackerCount = data[attacker] || 0;

        console.log(`   ${target} has ${targetCount} cookies`);
        console.log(`   ${attacker} has ${attackerCount} cookies`);

        // Check if target has enough cookies
        if (targetCount < amount) {
            console.log(`   Attack failed: ${target} doesn't have enough cookies`);
            return res.status(400).json({
                success: false,
                error: 'Target does not have enough cookies',
                targetCookies: targetCount
            });
        }

        // Update target - attacker loses nothing and gains nothing
        data[target] = targetCount - amount;

        // Ensure attacker exists in data
        if (!data.hasOwnProperty(attacker)) {
            data[attacker] = attackerCount;
        }

        fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));

        console.log(`💣 ATTACK SUCCESS: ${attacker} destroyed ${amount} cookies from ${target}`);
        console.log(`   ${target}: ${targetCount} → ${data[target]}`);
        console.log(`   ${attacker}: ${attackerCount} (no change)`);
        console.log(`   Updated data:`, data);

        res.json({
            success: true,
            stolen: amount,
            attackerNewCount: attackerCount,
            targetNewCount: data[target]
        });
    } catch (error) {
...
app.post('/api/cookie-clicker/:user', (req, res) => {
    try {
        const user = req.params.user;
        ...
        console.log(`Cookie clicker data saved for ${user}: ${count} cookies`);
        res.json({ success: true, count: data[user] });
    } catch (error) {
        console.error('Error saving cookie data:', error);
        res.status(500).json({ success: false, error: 'Failed to save cookie count' });
    }
});

// Cookie Clicker Upgrades API endpoints
app.get('/api/cookie-clicker-upgrades/:user', (req, res) => {
    try {
        const user = req.params.user;
        const dataPath = path.join(DATA_DIR, 'cookie-clicker-upgrades.json');
        
        if (fs.existsSync(dataPath)) {
            const data = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
            res.json(data[user] || { autoClicker: 0, doubleClick: 0, cookieFactory: 0 });
        } else {
            res.json({ autoClicker: 0, doubleClick: 0, cookieFactory: 0 });
        }
    } catch (error) {
        console.error('Error reading upgrade data:', error);
        res.json({ autoClicker: 0, doubleClick: 0, cookieFactory: 0 });
    }
});

app.post('/api/cookie-clicker-upgrades/:user', (req, res) => {
    try {
        const user = req.params.user;
        ...
        console.log(`Cookie clicker upgrades saved for ${user}:`, upgrades);
        res.json({ success: true, upgrades: data[user] });
    } catch (error) {
        console.error('Error saving upgrade data:', error);
        res.status(500).json({ success: false, error: 'Failed to save upgrades' });
    }
});

// Attack Items API endpoints
app.get('/api/attack-items/:user', (req, res) => {
    try {
        const user = req.params.user;
        const dataPath = path.join(DATA_DIR, 'attack-items.json');
        
        if (fs.existsSync(dataPath)) {
            const data = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
            res.json(data[user] || { bomb: 0, nuke: 0, marcosRage: 0, cawcaw: 0 });
        } else {
            res.json({ bomb: 0, nuke: 0, marcosRage: 0, cawcaw: 0 });
        }
    } catch (error) {
        console.error('Error reading attack items data:', error);
        res.json({ bomb: 0, nuke: 0, marcosRage: 0, cawcaw: 0 });
    }
});

app.post('/api/attack-items/:user', (req, res) => {
    try {
        const user = req.params.user;
        ...
        console.log(`Attack items saved for ${user}:`, items);
        res.json({ success: true, items: data[user] });
    } catch (error) {
        console.error('Error saving attack items data:', error);
        res.status(500).json({ success: false, error: 'Failed to save attack items' });
    }
});
